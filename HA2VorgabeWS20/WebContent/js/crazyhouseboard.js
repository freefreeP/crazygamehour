...
function validMove(move) {
  ...
  return ((validSquare(tmp[0]) === true || validPieceCode(tmp[0]) === true) && validSquare(tmp[1]) === true);
}
...
function validPieceCode(code) {
  ...
  return (code.search(/^[kqrnbpKQRNBP]$/) !== -1);
}
...
function validFen(fen) {
  ...
  for (var i = 0; i < BOARD_SIZE; i++) {
	if (chunks[i].search(/^[kqrnbpKQRNBP1-8]*/) === -1) {
	      return false;
	}
  }
...
function fenToObj(fen) {
	...
	position[fenToPieceCode('B')] = 0
	position[fenToPieceCode('r')] = 0
	position[fenToPieceCode('R')] = 0
	position[fenToPieceCode('n')] = 0
	position[fenToPieceCode('N')] = 0
...
/* Crazyhouse: not for us. User must see spare pieces even when not her turn
  if (cfg.sparePieces === true) {
    cfg.draggable = true;
  }
*/
...
function stopDraggedPiece(location) {
  ...
  if (cfg.hasOwnProperty('onDrop') === true &&
    ...
    var result = cfg.onDrop(DRAGGED_PIECE_SOURCE, location, pieceCodeToFen(DRAGGED_PIECE),
